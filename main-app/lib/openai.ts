...
export async function enhanceImagePrompt(prompt: string, hasCharacterReference: boolean): Promise<string> {
  ...
  const systemContent = hasCharacterReference
    ? `You are an expert image prompt engineer specializing in creating cinematic scenes. Your task is to enhance the user's prompt to be more epic and fantastic. When a character is present, describe them in rich detail—including appearance, expression, attire, and action—but it is just as important to create a vivid and compelling environment around them. The background should complement the character and the overall mood. Focus on dynamic lighting, atmospheric effects, and detailed scenery to create a high-quality, cinematic image. The output should be a single, enhanced prompt string, ready for an image generation model. Do not add any extra conversational text or formatting.`
